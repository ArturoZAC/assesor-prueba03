...
import { ResaltarFila, TablaGeneralLegacy } from "@/components/tables/TablaGeneraLegacy";
...
export default async function page({ searchParams }: { searchParams: any }) {
  ...
  const res = await fetch(
    `${config.apiUrl}/facturacion?page=${safePage}
      &limit=${limit}
  ...
  const datosConResaltar: FacturacionFila[] = datos.map((row: any) => ({
    ...row,
    resaltarFila: {
      active: row.resaltarFila?.active ?? false,
      data: row.resaltarFila?.data ?? [],
    },
  }));
